...
;; Make all time-grid entries, including ones from :LOGBOOK:, look the same
      ...
      (catch :skip
        ...
        (setq effort-minutes (when effort (save-match-data (org-duration-to-minutes effort))))
	      (when (string-match "\\]" timestr)
            ...
            ;; THIS IS CHANGED from a single dash "-" to "CLOCKED"
	          (setq clocked "CLOCKED")))
	      (save-excursion
	        (setq extra
		            (cond
